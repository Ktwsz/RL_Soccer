...
class EnvironmentBase(object):
    ...
    def update_state(self):
        ...
        for i in range(1, 3):
            self.state[3*self.old_pos[0]+i*incr[0]][3*self.old_pos[1]+i*incr[1]] = 0
    ...
    def reset(self):
        ...
        self.state = [[ 255 for i in range((self.height-1)*3+1)] for j in range((self.width-1)*3+1)]
        for i in range(1, self.width-1):
            for j in range(1, self.height-1):
                self.state[i*3][j*3] = 0
        ...
        for i in range(3*(self.gate0[1][0])+1):
            for j in range(3):
                self.state[i][3-j] = 0
                self.state[i][-4+j] = 0
        for i in range(3*(self.gate0[2][0]), 3*(self.width-1)+1):
            for j in range(3):
                self.state[i][3-j] = 0
                self.state[i][-4+j] = 0
