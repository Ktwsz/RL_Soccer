...
class ModelBase(nn.Module):
    ...
    def policy(self, env, debug=False):
        ...
        if self.epsilon > np.random.rand():
            action = self.policy_random(8)
        else:
            action = self.policy_greedy(actions, env.player)
        reward, next_state, done = env.action(action)
        self.add_experience(state, action, reward, next_state, done)

        return done

    def policy_greedy(self, actions, player):
        action_val = -1*inf
    ...
    def policy_random(self, size):
        return int(np.random.uniform()*size)